...
export default function SiteDetail() {
  ...
  const fallbackPrimary = {
    ...
    description:
      projectType === "carbon"
        ? "Satellite-derived canopy cover percentage (Sentinel-2)"
        : "Bio-acoustic & field survey biodiversity index",
  ...
  const fallbackSecondary = {
    ...
    description:
      projectType === "carbon"
        ? "Monthly estimated above-ground biomass carbon increment"
        : "Monthly unique flora & fauna observations",
        ...
        <Card className="grid grid-cols-1 md:grid-cols-4 gap-4 divide-y md:divide-y-0 md:divide-x divide-border">
          <div className="flex flex-col gap-1 pr-2">
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <span
                className={`text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded ${typeStyles[projectType]}`}
              >
                {projectType}
              </span>
              <span
                className={`text-[10px] font-medium capitalize px-2 py-0.5 rounded-full ${statusStyles[project.status || "active"]}`}
              >
          ...
          <div className="flex flex-col gap-1 pt-3 md:pt-0 md:pl-4">
            ...
            <div className="flex items-center gap-1 text-sm font-semibold text-ink">
              ...
              <span>
                {projectType === "carbon"
                  ? "Sentinel-2 + Field Transect"
                  : "Bio-acoustic + Camera Traps"}
              </span>
            </div>
            <span className="text-xs text-ink-muted mt-0.5">
            ...
            <Card className="flex flex-col justify-between h-full">
              <div className="border-b border-border pb-3">
                ...
                <p className="text-xs text-ink-muted mt-0.5">
                  Methodology notes and field records
                </p>
              </div>
